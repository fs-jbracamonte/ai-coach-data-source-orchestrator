        ...
        for (const section of sections) {
          try {
            await db.insert(schema.dailyReports)
              ...
              .onConflictDoUpdate({
                target: [schema.dailyReports.projectId, schema.dailyReports.authorName, schema.dailyReports.reportDate],
                set: {
                  content: section.content,
                  checksumSha256: section.checksum,
                  blobKey: blobRef?.blob_key || null,
      ...
      for (const filePath of files) {
        ...
        try {
          ...
          await db.insert(schema.meetingTranscripts)
            ...
            .onConflictDoUpdate({
              target: [schema.meetingTranscripts.projectId, schema.meetingTranscripts.filename, schema.meetingTranscripts.transcriptDate],
              set: {
                transcriptText: transcriptText,
                blobKey: blobRef.blob_key,
                blobUrl: blobRef.blob_url,
