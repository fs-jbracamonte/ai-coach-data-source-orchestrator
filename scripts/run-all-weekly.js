...
for (const team of teams) {
  ...
  if (res.error) {
    console.error(`[orchestrator] Error spawning ${team}:weekly →`, res.error.message);
    process.exit(1);
  }
  console.log(`[orchestrator] ${team}:weekly exited with status ${res.status}`);
  if (res.status !== 0) {
    console.error(`[orchestrator] Aborting due to non-zero exit for ${team}:weekly`);
...
console.log('\n✓ All weekly workflows completed.');
